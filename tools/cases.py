...
CASES = [
    ...
    },
    {
        ...
        "checks": ["catalog-number-is-integer", "omm-formats-agree", "tle-format-omits-ids-above-99999", "empty-answer-yields-no-records", "csv-json-omit-constant-metadata", "xml-ndm-wrapper-omm-2.0", "sha256-matches-tested-snapshot"],
    ...
    },
    {
        ...
        "checks": ["tle-format-omits-ids-above-99999", "empty-answer-yields-no-records", "tle-count-equals-omm-count-below-100000", "catalog-number-is-integer"],
    ...
    },
    {
        ...
        "checks": ["object-id-may-be-empty", "object-name-unknown-literal", "catalog-number-is-integer", "tle-format-omits-ids-above-99999", "empty-answer-yields-no-records",
    ...
    },
    {
        ...
        "checks": ["nine-digit-ids-parse", "catalog-number-is-integer", "omm-formats-agree", "tle-format-omits-ids-above-99999", "empty-answer-yields-no-records", "supgp-extra-keys-tolerated", "classification-c"],
    ...
    },
    {
        ...
        "coverage": {"provides": ["79 decaying objects with non-zero second derivative", "negative BSTAR (69999: -70517-5) and negative first derivative", "BSTAR up to 0.0052 (51949-2)"],
                     "gaps": ["no BSTAR with a positive exponent (>= 1.0 Earth radii^-1) exists in any fetched data; that encoding ('NNNNN+1') is untested on the reader side (the writer case carries a synthetic-derived vector for it, D-125)", "no second derivative with a positive exponent"]},
        "ambiguities": ["ecc-truncation-vs-mantissa-rounding", "mmdot-convention"],
    },
    {
    ...
    },
    {
        ...
        "synthetic_field_vectors": [  # D-125: field forms no fetched record supplies, rendered by the corpus; specification vectors, not catalogued objects
            {"from_case": "six-digit-omm-saramago", "set": "saramago-first", "norad_cat_id": 99999, "field": "bstar", "value": "1.2345",
             "expected_tle_field": " 12345+1",
             "id_vector_source": "vectors/alpha5.json below_100000 (99999, the largest five-digit number; primary sources describe no catalogued object in 90000-99999, README; rffit uses it as a placeholder)",
             "value_vector_source": "the TLE exponent field's positive-exponent form, +-NNNNN+E with E >= 1 (CelesTrak TLE format description); 0.12345E+1 = 1.2345 chosen as the smallest-digits example",
             "reason": "no fetched record carries a BSTAR with a positive exponent (audit of 2026-09-23, item 6: 1,206 lines, 1,192 negative exponents, 14 '+0', none >= 1.0)",
             "note": "Synthetic-derived under the D-096 precedent (D-125): SARAMAGO's real elements with the catalog number replaced by the five-digit vector 99999 and BSTAR replaced by 1.2345. A specification vector for the positive-exponent field form, not a catalogued object's element set; the expected TLE fields are rendered by the corpus."}],
